...
for maj_idx, min_idx in kfcv.split(X):
    ...
    fold_no+=1
print("mean performance for each dimensionality of feature space is ", np.mean(acc_list_knn, axis =1))
...
for maj_idx, min_idx in kfcv.split(X):
    ...
    for n_idx in range(0,len(feat_ns)):
        ...
        acc_list_mlr[n_idx][fold_no] = accuracy_score(y_min,mlr.predict(feat_selector.transform(X_min)))
...
for maj_idx, min_idx in kfcv.split(X):
    ...
    fold_no+=1
    print("new fold")
print("mean performance for each dimensionality of feature space is ", np.mean(acc_list_xgb, axis =1))
